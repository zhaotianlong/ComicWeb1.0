		...
		<div class="subscriptionContent">
			<div class="subscriptionTitle">
				<span>章节名:</span><input type="text" id="partName" value="${partName}" disabled="disabled" />
				 ...
				 <a id="partNameMdf"
					onclick="Modify('partName')">修改</a> 
					<a id="partNameConf"
					onclick="Config('partName')" style="display: none;">确认</a> 
					<a id="partNameCac" onclick="Cancel('partName')" style="display: none;">取消</a>
			...
			<div class="pageManage">
				<ul>
					<c:forEach var="page" items="${pages}">
						<li>
						<img id="${page.id}" src="<%=request.getContextPath()%>/${page.filePath}" width="80" height="80" />
						<div class="div1">
...
function UpLoadImg(id){
	...
	oMyForm.append("fileImg",file);
	 ...
	 xhr.open("POST","<%=request.getContextPath()%>/author/modifypageimg", true);
	 xhr.send(oMyForm); 
}
...
function GetFilePath(id,previewId) {
	...
	function SelectedPage() {
		var urlStr="<%=request.getContextPath()%>/author/modifycomicpage/${comicPartId}";	
	...
	function Config(inputId){
		...
		var urlStr="<%=request.getContextPath()%>/author/modifycomicpartname";	
		$.ajax({
			url : urlStr,
			async : false,
