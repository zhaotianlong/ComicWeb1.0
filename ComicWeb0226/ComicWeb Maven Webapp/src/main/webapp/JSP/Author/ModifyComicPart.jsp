	...
	<div class="subscription">
		...
		<div class="subscriptionContent">
			<div class="subscriptionTitle">
				...
				<a class="textRight buttton" onclick="AddComicPage('${comicId}')">新增</a>
			</div>
			...
			<table cellspacing="0">
				...
				<c:forEach var="comicpart" items="${comicparts}">
					<tr>
						...
						<td class="td_1">${comicpart.partName}</td>
						<td class="th_3"><a onclick="ModifyComicPage('${comicpart.partId}')">修改</a><a>清空</a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
...
function AddComicPage(id){
	var urlStr="<%=request.getContextPath()%>/author/addcomicpage";
		urlStr += "/" + id;
		$.ajax({
			url : urlStr,
			async : false,
...
function ModifyComicPage(id){
	var urlStr="<%=request.getContextPath()%>/author/modifycomicpage";
			urlStr += "/" + id;
			$.ajax({
				url : urlStr,
				async : false,
...
function SelectedPage() {
	var urlStr="<%=request.getContextPath()%>/author/modifycomicpart/${comicId}";	
