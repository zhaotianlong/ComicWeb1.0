		...
		<aside id="loveComicer" class="right">
			<div class="inerestWriter" style="width: 100%;">
				...
				<div class="inerestWriterContetnt">
					<div class="interestwTitle">共<span class="fontRed">${userauthor_total}</span>条 评论     <a onclick="openPopup('openModal2')">新增关注</a></div>
					<table>
						<tr>
							<th class="td_1"><input type="checkbox" /></th>
							<th class="th_1">关注id</th>
						...
						<c:forEach var="tmp" items="${userauthors}">
						<tr id="${tmp.id}">
							<th class="td_1"><input type="checkbox" /></th>
							<td class="th_1">${tmp.id}</td>
							<td class="th_2">${tmp.userId.accountId}</td>
							<th class="th_2">${tmp.userId.nickName}</th>
							<td class="th_2">${tmp.authorId.authorId}</td>
							<th class="th_2">${tmp.authorId.realName}</th>
							<th class="th_2">
					...
					<div class="subscriptionFooter">
						...
						<c:forEach var="i" begin="1" end="${userauthor_sum}" step="1">
							<c:choose>
								<c:when test="${i==userauthor_currentPage}">
						...
						<a onclick="GoOnePage(${userauthor_sum})" class="button buttonWhite">尾页</a>
					</div>
				</div>
			</div>
<div id="openModal1" class="modalDialog">
	<div>
		...
		<div>修改关注</div>
						<form action="modifyuserauthor/do" class="profileContent" id="modifyform">
			...
			<div class="modifyDiv">
				...
				<input type="text" id="param3" value=""
					...
					type="hidden" id="param3Hidden" name="authorId"
					value="" />
			</div>
...
<div id="openModal2" class="modalDialog">
	<div>
		<div onclick="closePopup('openModal2')" title="Close" class="close">X</div>
					<div>新增关注</div>
				<form action="createuserauthor/do" class="profileContent" id="modifyform">
			<div class="modifyDiv">
			...
			<div class="modifyDiv">
				作者id:<input type="text" id="comicId" value=""
					...
					type="hidden" id="comicIdHidden" name="authorId"
					value="" />
			</div>
			<div class="modifyDiv" style="text-align: center;">
				<input type="submit" value="提交" class="button" />
	...
	function SelectedPage() {
		var urlStr="<%=request.getContextPath()%>/cms/userauthormanage";	
	...
	function GoOnePage(num){
		var urlStr="<%=request.getContextPath()%>/cms/userauthormanage";	
		if(num<1)num=1;
		if(num>${userauthor_sum})num=${userauthor_sum};
		var currentNum =num;
			//alert(currentNum);
			urlStr+="/"+currentNum;
			$.ajax({
	...
	function CancelComment(id){
		var urlStr="<%=request.getContextPath()%>/cms/canceluserauthor";	
		var num=$("#selectPage").val();
		$.ajax({
				url : urlStr,
				async : false,
