	...
	<table>
		...
		<c:forEach var="comment" items="${comments}">
			<tr>
				...
				<td class="th_3">
				<a href="<%=request.getContextPath()%>/comic/comiclist/${comment.comicId}">查看</a><a href="" onclick="CannelCommentBack('${comment.subid}')">撤回</a></td>
			</tr>
		</c:forEach>
	</table>
	 -->
	...
	<section class="comments">
		...
		<article class="comment">
			...
			<div class="comment-body">
				...
				<p class="attribution">在 <a href="#non">${comment.comicName}中  你回复${comment.userName}说     </a>时间:${comment.newDate} </p>
			</div>
		</article>
		</c:forEach>
	</section>
...
function CannelCommentBack(id){
	var urlStr="<%=request.getContextPath()%>/personcenter/message/commentbackfrom/cancel";
	urlStr+="/"+id;
	$.ajax({
		url : urlStr,
		async : false,
