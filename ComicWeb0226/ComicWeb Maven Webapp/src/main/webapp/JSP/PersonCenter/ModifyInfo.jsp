...
<aside id="modifyInfo" class="right" >
			<div class="profile">
				...
				<form:form modelAttribute="UserInfoModel" action="modifyinfo/do" cssClass="profileContent" id="modifyform">
					...
					<div>账号:<span><%= ((User)request.getSession().getAttribute("user")).getAccountId()%></span></div>
					<div>
					...
					<div>性别:
						<c:choose>
							<c:when test="${UserInfoModel.gender=='M'}">
								<span>男</span><input type="radio" name="gender" checked="checked" value="M" />
								<span>女</span><input type="radio" name="gender"   value="W" />
								
							</c:when>
							<c:otherwise>
								<span>男</span><input type="radio" name="gender" value="M" />
			...
			function Config(inputId){
				...
				function Reset(){
					var urlStr="<%=request.getContextPath()%>/personcenter/modifyinfo";
					$.ajax({
						url : urlStr,
						async : false,
						error : function() {
